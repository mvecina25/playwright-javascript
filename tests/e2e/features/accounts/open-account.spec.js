    ...
    test(
        ...
        async ({ userCreationFixture, loginPage, homePage, openAccountPage }) => {
            ...
            await test.step('THEN a success message should appear and a numeric account ID should be captured', async () => {
                 ...
                 */
                expect(savingsAccountId, 'The generated Account ID should be numeric').toMatch(
                    /^\d+$/,
                );
    ...
    test(
        ...
        async ({ savingsAccountCreationFixture, basePage, accountActivityPage }) => {
            ...
            await test.step('WHEN the user navigates to the account activity details', async () => {
                ...
                await expect(
                    accountActivityPage.accountDetailsTitle,
                    'Heading should confirm we are in Account Details',
            ...
            await test.step('THEN the account type and initial balance should match the system defaults', async () => {
                ...
                expect(await accountActivityPage.getAccountIdText()).toBe(
                    savingsAccountCreationFixture.savingsAccountId,
                );

                // Verify the account categorization
                expect(await accountActivityPage.getAccountTypeText()).toBe(ACCOUNT_TYPE_SAVINGS);
                 ...
                 */
                expect(balanceText, 'Balance should follow currency format').toMatch(
                    /^\$\d+\.\d{2}$/,
                );
                expect(
                    balanceText,
                    'New savings accounts must start with the system default balance',
                ).toBe(INITIAL_ACCOUNT_BALANCE);
            });
        },
    );
});
